...
{
  if (addr < 0x4000)
    return mem->cartridge_memory[0x4000 * slot1 + addr];
  if (addr < 0x8000)
    return mem->cartridge_memory[0x4000 * slot2 + (addr - 0x4000)];
  if (addr < 0xC000)
  {
    if (ram_slot3)
      return mem->ram_banks[ram_bank][addr - 0x8000];
    else
      return mem->cartridge_memory[0x4000 * slot3 + (addr - 0x8000)];
  }
