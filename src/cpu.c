...
u8 *get_register(u8 inst)
{
  ...
  {
  case 0:
    return &cpu->main.singles.B;
  case 1:
    return &cpu->main.singles.C;
  case 2:
    return &cpu->main.singles.D;
  case 3:
    return &cpu->main.singles.E;
  case 4:
    return &cpu->main.singles.H;
  case 5:
    return &cpu->main.singles.L;
  case 7:
    return &cpu->main.singles.A;
  }
  return 0;
}
...
int execute_cb()
{
  ...
  if (reg)
    ...
    input = read_u8(cpu->main.pairs.HL);

  u8 result = execute_bitwise(inst, input);
  if (reg)
    *reg = result;
  else
    write_u8(cpu->main.pairs.HL, result);
...
int execute_displacedcb(u16 *reg)
{
  ...
  if (resultReg)
    *resultReg = result;
    ...
    write_u8(displacedAddress, result);

  if (inst < 0x80 && inst >= 0x40)
    return 20;
  return 23;
}